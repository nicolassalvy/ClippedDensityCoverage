"""
Original implementation from the authors of Probabilistic Precision and Recall,
taken from their github:
https://github.com/kdst-team/Probablistic_precision_recall/blob/master/metric/pp_pr.py

Dogyun Park and Suhyun Kim. Probabilistic precision and recall towards reliable
evaluation of generative models. In Proceedings of the IEEE/CVF international
conference on computer vision, pages 20099–20109, 2023.
"""
...
def get_pairwise_distances(real, fake, jobs=8, gpu=False):
    if not gpu:
        pairwise_distance_between_real = pairwise_distances(
            real, real, n_jobs=jobs, metric="l2"
        )
        pairwise_distance_between_fake = pairwise_distances(
            fake, fake, n_jobs=jobs, metric="l2"
        )
        pairwise_distance_between_real_fake = pairwise_distances(
            real, fake, n_jobs=jobs, metric="l2"
        )
        pairwise_distance_between_fake_real = np.transpose(
        ...
        pairwise_distance_between_fake_real = torch.transpose(
            pairwise_distance_between_real_fake, 0, 1
        )

    return (
        pairwise_distance_between_real,
...
def compute_pprecision_precall(real, fake, a=1.2, kth=4, gpu=False):
    ...
    pairwise_distance_real, pairwise_distance_fake, distance, distance_t = (
        get_pairwise_distances(real, fake, gpu=gpu)
    )

    # Calculate maximum distance for f
    k_nearest_x = get_average_of_knn_distance(
        pairwise_distance_real, kth, gpu=gpu
    )
    k_nearest_y = get_average_of_knn_distance(
        pairwise_distance_fake, kth, gpu=gpu
    )
