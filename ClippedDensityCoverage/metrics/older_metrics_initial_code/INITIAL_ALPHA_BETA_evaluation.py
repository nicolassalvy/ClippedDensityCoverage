"""
Original implementation from the authors of alpha-Precision and beta-Recall,
taken from their github:
https://github.com/vanderschaarlab/evaluating-generative-models/blob/main/metrics/evaluation.py
...
commented out the import of logging and scipy
...
def compute_alpha_precision(real_data, synthetic_data, emb_center):
    ...
    Radii = np.quantile(
        torch.sqrt(
            torch.sum(
                (torch.tensor(real_data).float() - emb_center) ** 2, dim=1
            )
        ),
        alphas,
    )
    ...
    synth_to_center = torch.sqrt(
        torch.sum(
            (torch.tensor(synthetic_data).float() - emb_center) ** 2, dim=1
        )
    )
    ...
    nbrs_synth = NearestNeighbors(n_neighbors=1, n_jobs=-1, p=2).fit(
        synthetic_data
    )
    ...
    real_synth_closest_d = torch.sqrt(
        torch.sum(
            (torch.tensor(real_synth_closest).float() - synth_center) ** 2,
            dim=1,
        )
    )
    ...
    for k in range(len(Radii)):
        precision_audit_mask = (
            (synth_to_center <= Radii[k]).detach().float().numpy()
        )
