...
def data_to_tensors(x, x_len, x_rev, x_rev_shifted, float_type, device):
    ...
    X_rev_shifted = torch.tensor(
        x_rev_shifted, device=device, dtype=float_type
    )
...
def _generate_dummy_data(
    n_samples, min_timesteps, max_timesteps, n_features, pad_val, seed
):
    np.random.seed(seed)

    seq_lengths = np.random.randint(
        low=min_timesteps, high=max_timesteps + 1, size=n_samples
    )
