...
class Encoder(nn.Module):
    ...
    def forward(self, x, x_seq_lengths, hc, padding_value, max_seq_len):
        ...
        x, hc = self.lstm(x, hc)
        x, x_seq_lens = pad_packed_sequence(
            x,
            batch_first=True,
...
class Seq2Seq(nn.Module):
    ...
    def forward(
        self, x_enc, x_dec, x_seq_lengths, hc_init, padding_value, max_seq_len
    ):
        # print(x_enc.dtype, x_dec.dtype, x_seq_lengths.dtype, hc_init[0].dtype, hc_init[1].dtype)
        x_enc_out, _, hc_enc = self.encoder(
            x_enc, x_seq_lengths, hc_init, padding_value, max_seq_len
        )
        # print("x_enc.shape", x_enc.shape)
        # print("x_enc_out.shape", x_enc_out.shape)
        x_dec_out, _, hc_dec = self.decoder(
            x_dec, x_seq_lengths, hc_enc, padding_value, max_seq_len
        )
        return x_dec_out, hc_enc

    def get_embeddings_only(
        self, x_enc, x_seq_lengths, hc_init, padding_value, max_seq_len
    ):
        _, _, hc_enc = self.encoder(
            x_enc, x_seq_lengths, hc_init, padding_value, max_seq_len
        )
        return hc_enc
