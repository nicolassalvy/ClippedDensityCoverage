...
try:
    ...
    print(
        "IPython not found, ts_embedding > training live plot will not work."
    )
...
def train_seq2seq_autoencoder(
    ...
    train_losses, val_losses = np.full([n_epochs], np.nan), np.full(
        [n_epochs], np.nan
    )
    ...
    for epoch in range(n_epochs):
        ...
        seq2seq.train()
        n_samples_train = 0
        for iter_, (x, x_len, x_rev, x_rev_shift) in enumerate(
            train_dataloader
        ):
            batch_size = x.shape[0]
        ...
        if jupyter_live_plot_enabled or (
            not jupyter_live_plot_enabled and epoch == n_epochs - 1
        ):
        ...
        print(
            f"Epoch {epoch}: Tr.Ls.={epoch_train_loss:.3f} Vl.Ls.={epoch_val_loss:.3f}"
        )
