...
class OneClassLayer(BaseNet):
    ...
    def __init__(self, params=None, hyperparams=None):
        ...
        self.model = build_network(
            network_name="feedforward", params=params
        ).to(self.device)
    ...
    def fit(self, x_train, verbosity=True):
        ...
        for epoch in range(self.epochs):
            ...
            inputs = (
                Variable(torch.from_numpy(x_train)).to(self.device).float()
            )
            ...
            if (epoch >= self.warm_up_epochs) and (
                self.loss_type == "SoftBoundary"
            ):
            ...
            if self.train_prop != 1.0:
                with torch.no_grad():
                    ...
                    if self.loss_type == "SoftBoundary":
                        ...
                        loss_val = (
                            self.loss_fn(outputs=outputs, c=self.c)
                            .detach.cpu()
                            .numpy()
                        )
