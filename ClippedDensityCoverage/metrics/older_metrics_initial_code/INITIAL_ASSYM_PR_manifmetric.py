"""
Original implementation from the authors of Symmetric Precision and Recall,
taken from their github:
https://github.com/mahyarkoy/emergent_asymmetry_pr/blob/main/manifmetric/manifmetric.py

Mahyar Khayatkhoei and Wael AbdAlmageed. Emergent asymmetry of precision and
recall for measuring fidelity and diversity of generative models in high
dimensions. In International Conference on Machine Learning, pages 16326–16343.
PMLR, 2023.
"""
...
class VGG16(nn.Module):
    ...
    def forward(self, in_):
        in_t = FT.resize(
            in_, 256, interpolation=InterpolationMode.BILINEAR, antialias=None
        )
        in_t = FT.center_crop(in_t, 224)
        in_t = FT.normalize(
            in_t, mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]
        )
...
class ManifoldMetric:
    def __init__(self, model=None, ref_data=None, gen_data=None, k=None):
        ...
        """
        self.ref_stats = (
            self.compute_ref_stats(ref_data, k=k)
            if ref_data is not None
            else None
        )
        self.gen_stats = (
            self.compute_gen_stats(gen_data, k=k)
    ...
    def extract_features(
        ...
        for data in dataloader:
            in_tensor = torch.as_tensor(
                transform(data), dtype=dtype, device=device
            )
            out = model(in_tensor)
        ...
        features = (
            torch.cat(features)[:num_samples]
            if num_samples is not None
            else torch.cat(features)
        )
        if output_shape is not None and output_ids is not None:
            assert (
                len(output_ids) == features.shape[0]
            ), f">>> {len(output_ids)} != {features.shape}"
            output = torch.zeros(
    ...
    def compute_knn(self, source, target, k):
        ...
        target = torch.as_tensor(
            target, dtype=source.dtype, device=source.device
        )
        size = source.shape[0]
        k = min(k, target.shape[0])
        assert k > 0

        dist2 = torch.as_tensor(
            torch.zeros((size, k)), dtype=source.dtype, device=source.device
        )
        dist2_args = torch.as_tensor(
            torch.zeros((size, k)), dtype=torch.int64, device=source.device
        )
        for i in range(size):
            sort_dist2, sort_args = torch.sort(
                (source[i] - target).pow(2).flatten(1).sum(-1)
            )
    ...
    def coverage(self, ref_stats=None, gen_stats=None, k=5):
        ...
        gen_data = torch.as_tensor(
            gen_stats.data,
            dtype=ref_stats.data.dtype,
            device=ref_stats.data.device,
        )
        collect = 0
    ...
    def sym_precision(self, ref_stats=None, gen_stats=None, k=5):
        ...
        return DotDict(
            sym_precision=min(prec, comp_prec),
            precision=prec,
            comp_precision=comp_prec,
        )
    ...
    def sym_recall(self, ref_stats=None, gen_stats=None, k=5):
        ...
        return DotDict(
            sym_recall=min(recall, coverage), recall=recall, coverage=coverage
        )
