...
def plot_multiple_normalized_metrics_with_legend(
    ...
    for i, metric in enumerate(metric_names):
        ...
        color = colors[i] if not metric_name.endswith("(ours)") else "red"
        (line,) = ax.plot(
            x_values,
            normalized_means[i],
            label=metric_name,
...
def plot_ood_proportion_test(
    ...
    for i, metric in enumerate(metrics_of_interest):
        ...
        if where == "synthetic":
            ...
            ylim = (
                min(
                    min_val - 0.01,
                    reference_value - ood_proportions[-1],
                ),
                max(
                    max_val + 0.01,
                    reference_value + ood_proportions[-1],
    ...
    if categories is not None:
        ...
        for category in ["fidelity", "coverage"]:
            ...
            if where == "both":
                min_val = np.min(
                    normalized_mean[category_indices]
                    - normalized_std[category_indices]
                )
                max_val = np.max(
                    normalized_mean[category_indices]
                    + normalized_std[category_indices]
